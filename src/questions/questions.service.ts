...
export class QuestionsService implements OnModuleInit {
  ...
  async onModuleInit() {
    // Seed de preguntas iniciales si no existen
  ...
  async findAll(difficulty?: string): Promise<Question[]> {
  ...
  async getRandomQuestion(difficulty?: string): Promise<QuestionDocument> {
    const filter = difficulty ? { difficulty } : {};
    ...
    if (!question) {
      throw new NotFoundException('No se pudo obtener una pregunta');
    }
